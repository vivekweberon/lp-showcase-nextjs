...
let origin =
  "&origin=" + encodeURIComponent("https://" + window.location.hostname);
...
function renderUI(id) {
  ...
  $.get(
    LISTINGS_ROOT_DIR + "/" + DATA_DIR + "/" + id + "/" + YAML_FILENAME,
    function (data) {
  ...
  if (menu != undefined && menu != "") {
    ...
    if (url != undefined) {
      ...
      if (menu != expectedMenu) {
        $(id).attr("id", anchorName);
      }
    }
    let nlText = menu;
    let hLink = $(
      '<a class="nav-item nav-link" href="' + nlHref + '">' + nlText + "</a>"
    );
    let fLink = $(
      '<div class="col-6 col-sm-4 col-md-3" style="text-align: center; text-decoration: underline"><a class="nav-item nav-link" href="' +
        nlHref +
        '" style="color: #212529">' +
        nlText +
        "</a></div>"
...
function addVirtualTour(uiConfig) {
  ...
  if (virtualTour) {
    ...
    if (
      virtualTour.matterportID != undefined &&
      virtualTour.matterportID != ""
    ) {
      let virtualTourURL =
        "https://my.matterport.com/show/?m=" + virtualTour.matterportID;
  ...
  if (photos && photos.urls) {
    ...
    for (let i = 0; i < photos.urls.length; ++i) {
      ...
      photo =
        ...
        ' src="' +
        src +
        '1920" class="d-block" style="margin:auto; object-fit:contain" sizes="100vw" onerror="logResourceLoadError(this)">' +
        "</div>";

      $("#photosCarouselInner").append(photo);
      indicator =
        '<li data-target="#photosCarousel" data-slide-to="' +
        i +
        '" class="' +
        iclass +
        '"></li>';
...
function addVideo(uiConfig) {
  ...
  if (
    ...
    $("#homeVideo").attr("src", videoURL + origin);
...
function addShowcase(uiConfig) {
  ...
  if (showcase && showcase.properties && showcase.properties.length > 0) {
    ...
    for (let i = 0; i < showcase.properties.length; ++i) {
      ...
      propertyPageURL = property.pageUrl.startsWith("index.html")
        ? LISTINGS_ROOT_DIR + "/?" + property.pageUrl.split("?")[1]
        : property.pageUrl;
      src = LISTINGS_ROOT_DIR + "/" + property.url + "?width=";
      content =
        '<div class="col-sm-6 col-md-4 col-lg-3" style="padding:2.5px">' +
        '<div class="card">' +
        "<img" +
        ' srcset="' +
        src +
        ...
        ' src="' +
        src +
        '1920" class="card-img-top" style="object-fit:contain; width:100%"' +
...
function addChatBot(uiConfig) {
  ...
  if (chatbot && chatbot.enable == true) {
    ...
    let chatbotCSS =
      ...
      LISTINGS_ROOT_DIR +
      '/css/chatbot.css" onerror="logResourceLoadError(this)">';
    ...
    let chatbotJS =
      '<script src="' +
      LISTINGS_ROOT_DIR +
      '/js/chatbot.js" onerror="logResourceLoadError(this)"></script>';
    $(document.body).append(chatbotJS);
    // Add index.js
    let indexJS =
      '<script src="' +
      LISTINGS_ROOT_DIR +
      '/js/index.js" onerror="logResourceLoadError(this)"></script>';
...
function generateUI(uiConfig) {
  if (
    uiConfig == undefined ||
    (Object.keys(uiConfig).length == 1 && "homePageData" in uiConfig)
  ) {
    return;
  }
  ...
  let order =
    pageType == "home"
      ? uiConfig.homePageSectionsOrder
      : uiConfig.propertyPageSectionsOrder;
