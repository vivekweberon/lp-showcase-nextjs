...
runBuilder() {
    ...
    node ./builder.js --websiteName "$WEBSITE_DIRECTORY_NAME" --siteName "$siteName" --config "$config" --messagesDir "$messagesDir" --mauticTrackerDir "$mauticTrackerDir" --dataDir "$dataDir" --publicDir "$publicDir" || { echo "Error: builder.js execution failed"; exit 1; }
